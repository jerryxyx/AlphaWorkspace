#!/usr/bin/env python3
"""
Cross‑method validation with Monte Carlo values across spot grid.
"""
import sys
...
def main():
    ...
    # Numerical settings (lightweight for speed)
    lattice_n = 50
    mc_n_paths = 300
    mc_n_steps = 15
    pde_S_min = 1.0
    ...
    for sigma_spot in sigma_spot_list:
        for sigma_offset in sigma_offset_list:
            for option_type in option_types:
                ...
                # 3) Monte Carlo values across spot grid (expensive)
                mc_vals = []
                for spot in spot_grid:
                ...
                early_premium_mc = mc_vals[spot_grid == S0][0] - euro
                ...
                rows.append({
                    ...
                    'mc_value_S0': mc_vals[spot_grid == S0][0],
    ...
    csv_path = 'output/cross_method_with_mc_summary.csv'
    ...
    for row, sigma_spot in enumerate(sigma_spot_list):
        ...
        for sigma_offset in sigma_offset_list:
            ...
            ax_call.scatter(call_data['spot_grid'], call_data['mc_vals'],
            ...
            ax_put.scatter(put_data['spot_grid'], put_data['pde_vals'],
                          marker='x', s=30, color=color, alpha=0.7)
            ax_put.scatter(put_data['spot_grid'], put_data['mc_vals'],
                          marker='+', s=60, color=color, linewidths=1.5)
    ...
    plt.savefig('output/cross_method_option_values_with_mc.png', dpi=150)
    plt.close()
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    plt.savefig('output/cross_method_premium_boundaries_with_mc.png', dpi=150)
    plt.close()
    
    print('✅ Generated plots:')
    print('   - cross_method_option_values_with_mc.png')
    print('   - cross_method_premium_boundaries_with_mc.png')
