...
- Proxy group status (GLOBAL selection, available proxies)
"""
...
def get_proxy_info():
    """Get proxy groups and selections."""
    if not os.path.exists(SOCKET_PATH):
        return None, 'socket_not_found'
    cmd = f'curl -s --unix-socket {SOCKET_PATH} -H "Authorization: Bearer {SECRET}" http://localhost/proxies'
    out, err, code = run_cmd(cmd)
    if code != 0 or not out:
        return None, f'api_error:{err}'
    try:
        ...
        builtin = {'DIRECT', 'REJECT', 'GLOBAL', 'COMPATIBLE', 'PASS', 'REJECT-DROP'}
        groups = {}
        for name, info in proxies.items():
            if name in builtin:
                continue
            if isinstance(info, dict) and 'now' in info:
                groups[name] = {
                    'now': info.get('now'),
                    'type': info.get('type'),
                    'all': info.get('all', [])[:5]
        ...
        return {
            'global_now': global_info.get('now'),
...
def main():
    ...
    # Proxy info
    proxy_info, proxy_detail = get_proxy_info()
    result['proxies']['data'] = proxy_info
    result['proxies']['detail'] = proxy_detail
    ...
    if proxy_info and isinstance(proxy_info, dict):
        global_now = proxy_info.get('global_now', '')
        summary_parts.append(f'GLOBAL: {global_now}')
